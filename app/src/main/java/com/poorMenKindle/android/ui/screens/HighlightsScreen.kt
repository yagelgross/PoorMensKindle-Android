            ...
            confirmButton = {
                TextButton(onClick = {
                    ...
                    coroutineScope.launch(Dispatchers.IO) {
                        ...
                        val dao = db.bookDao()
                        if (hl.id > 0) {
                            dao.updateHighlightNoteByServerId(hl.id, noteText)
                        } else {
                            dao.updateHighlightNote(-hl.id, noteText)
                        }
                        val updated = dao.getHighlightsForBook(bookId)
                        withContext(Dispatchers.Main) {
                            highlights = updated.map {
                                HighlightItem(it.serverHighlightId ?: -it.localId, it.chapterIndex, it.highlightedText,
                        ...
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            ...
                            IconButton(onClick = {
                                coroutineScope.launch(Dispatchers.IO) {
                                    ...
                                    if (highlight.id > 0) {
                                        try {
                                            val response = NetworkManager.api.deleteHighlight(highlight.id)
                                            if (response.isSuccessful) {
                                                dao.deleteHighlightByServerId(highlight.id)
                                            }
                                        } catch (e: Exception) {
                                            // Optional: Show error
                                        }
